...
from settings import (MAX_TOTAL_UPGRADES, VOLUME_BASE_EXPLOSAO_BOSS, VOLUME_BASE_EXPLOSAO_NPC, 
                      ...
                      VOLUME_BASE_TIRO_INIMIGO, VOLUME_BASE_TIRO_PLAYER, VIDA_POR_NIVEL
                      )
...
try:
    ...
    print(f"[AVISO] Erro ao carregar sons (jogo continuará sem alguns sons): {e}")
...
pygame.display.set_caption("Space Orbit - Multiplayer Refatorado")
clock = pygame.time.Clock()

try:
    s.LOGO_JOGO = pygame.image.load("Space_Orbit.png").convert_alpha()
except Exception:
    s.LOGO_JOGO = None 
...
# --- Funções Auxiliares de Reinício ---
...
def distribuir_atributos_bot(nave, pontos):
    ...
    for _ in range(pontos):
        ...
        if not nave.comprar_upgrade(upgrade_escolhido):
            for opt in opcoes:
                if nave.comprar_upgrade(opt): break
...
def reiniciar_jogo_pvp(is_online=False, pos_spawn=None): 
    ...
    num_estrelas_pvp = max(100, int(s.NUM_ESTRELAS * proporcao))
    
    for _ in range(num_estrelas_pvp): 
        pos_base = pygame.math.Vector2(random.randint(0, s.MAP_WIDTH), random.randint(0, s.MAP_HEIGHT))
        raio = random.randint(1, 2)
        lista_estrelas.append((pos_base, raio, raio * 0.1))
    ...
    for group in game_groups.values(): group.empty()

    if is_online:
        nave_player.nome = network_client.get_my_name()
        pos_spawn_vec = pygame.math.Vector2(pos_spawn[0], pos_spawn[1])
    else:
        nave_player.nome = game_globals["nome_jogador_input"].strip() or "Jogador"
    ...
    nave_player.lista_todas_auxiliares = [NaveAuxiliar(nave_player, pos) for pos in Nave.POSICOES_AUXILIARES]
    ...
    nave_player.pontos_upgrade_disponiveis = pvp_s.PONTOS_ATRIBUTOS_INICIAIS
    ...
    for _ in range(15):
        ...
        grupo_obstaculos.add(Obstaculo(x, y, random.randint(s.OBSTACULO_RAIO_MIN, s.OBSTACULO_RAIO_MAX)))
...
def reiniciar_jogo(pos_spawn=None, dificuldade="Normal"): 
    ...
    if pos_spawn:
        ...
        spawn_x = random.randint(100, s.MAP_WIDTH - 100)
        spawn_y = random.randint(100, s.MAP_HEIGHT - 100)
...
def respawn_player_offline(nave):
    ...
    nave.lista_todas_auxiliares = [NaveAuxiliar(nave, pos) for pos in Nave.POSICOES_AUXILIARES]
    ...
    nave.tempo_spawn_protecao_input = pygame.time.get_ticks() + 500
...
def ciclar_alvo_espectador(game_globals_dict, avancar=True):
    ...
    if network_client.is_connected():
        ...
        if game_globals_dict["estado_jogo"].startswith("PVP_"):
            if nave_player.vida_atual > 0: lista_alvos_vivos.append(nave_player)
            for bot in grupo_bots:
                if bot.vida_atual > 0: lista_alvos_vivos.append(bot)
        else:
            if nave_player.vida_atual > 0: lista_alvos_vivos.append(nave_player)
            for bot in grupo_bots:
                if bot.vida_atual > 0: lista_alvos_vivos.append(bot)
    ...
    if network_client.is_connected():
        game_globals_dict["alvo_espectador_nome"] = lista_alvos_vivos[current_index]['nome']
        game_globals_dict["alvo_espectador"] = None
    else:
        game_globals_dict["alvo_espectador"] = lista_alvos_vivos[current_index]
        game_globals_dict["alvo_espectador_nome"] = None

# --- Configuração de UI e Callbacks ---
...
game_globals = {
    ...
    "pvp_disponivel": True, 
    "pvp_lobby_num_players": 0, "pvp_lobby_countdown_sec": 0, "pvp_match_countdown_sec": 0, 
    "pvp_lobby_timer_fim_offline": 0, "pvp_partida_timer_fim_offline": 0, "pvp_pre_match_timer_fim_offline": 0, 
    "pvp_game_over_timer_fim": 0, "pvp_vencedor_nome": "Ninguém",      
...
while game_globals["rodando"]:
    ...
    LARGURA_ANTIGA = LARGURA_TELA; ALTURA_ANTIGA = ALTURA_TELA

    # --- NOVO: Verificação de Queda de Conexão ---
    # Se achávamos que estava online, mas o socket morreu, trata o erro.
    if is_online and not network_client.is_connected():
        print("[MAIN] Queda de conexão detectada! Redirecionando para tela de erro...")
        is_online = False
        game_globals["is_online"] = False
        ...
        # Reset visual
        nave_player.posicao = pygame.math.Vector2(s.MAP_WIDTH // 2, s.MAP_HEIGHT // 2)
        grupo_bots.empty(); grupo_inimigos.empty()
    ...
    if estado_jogo == "PVE_OFFLINE_START":
        ...
        estado_jogo = "JOGANDO"; game_globals["estado_jogo"] = "JOGANDO"
    ...
    if estado_jogo == "ESPECTADOR":
        ...
        if not target_found:
            ...
            teclas = pygame.key.get_pressed(); velocidade_espectador = 20
    ...
    if estado_jogo not in ["MENU", "GET_NAME", "GET_SERVER_INFO", "MULTIPLAYER_MODE_SELECT", "OFFLINE_MODE_SELECT", "PVE_OFFLINE_START", "PVP_OFFLINE_START", "TENTANDO_CONECTAR", "ERRO_CONEXAO"]:
        if is_online:
            ...
            if estado_jogo.startswith("PVP_") or (estado_jogo == "ESPECTADOR" and is_pvp_map):
                ...
                game_globals["pvp_match_countdown_sec"] = lobby_status.get("match_countdown_sec", 0)
                
                server_lobby_state = lobby_status.get("lobby_state", "WAITING")
                if server_lobby_state == "GAME_OVER" and estado_jogo != "PVP_GAME_OVER":
                        estado_jogo = "PVP_GAME_OVER"; game_globals["estado_jogo"] = "PVP_GAME_OVER"
                        game_globals["pvp_vencedor_nome"] = lobby_status.get("winner", "Erro")
                        game_globals["pvp_game_over_timer_fim"] = agora + 5000
                elif server_lobby_state == "PRE_MATCH" and estado_jogo != "PVP_PRE_MATCH":
                        estado_jogo = "PVP_PRE_MATCH"; game_globals["estado_jogo"] = "PVP_PRE_MATCH"
                        game_globals["pvp_pre_match_timer_fim_offline"] = agora + (5 * 1000)
                elif server_lobby_state == "PLAYING" and estado_jogo != "PVP_PLAYING":
                        estado_jogo = "PVP_PLAYING"; game_globals["estado_jogo"] = "PVP_PLAYING"
                elif server_lobby_state == "COUNTDOWN" and estado_jogo != "PVP_COUNTDOWN":
                        estado_jogo = "PVP_COUNTDOWN"; game_globals["estado_jogo"] = "PVP_COUNTDOWN"
                elif server_lobby_state == "WAITING" and estado_jogo != "PVP_LOBBY" and estado_jogo != "MENU":
                    if estado_jogo == "PVP_GAME_OVER":
                        resetar_para_menu(); estado_jogo = "MENU" 
                    else:
                        estado_jogo = "PVP_LOBBY"; game_globals["estado_jogo"] = "PVP_LOBBY"
            ...
            if my_state:
                ...
                if (estado_jogo == "ESPECTADOR" or estado_jogo == "PAUSE") and nave_player.vida_atual <= 0 and nova_vida > 0:
                    if s.MAP_WIDTH < 5000: estado_jogo = "PVP_LOBBY"; game_globals["estado_jogo"] = "PVP_LOBBY"
                    else: estado_jogo = "JOGANDO"; game_globals["estado_jogo"] = "JOGANDO"
                    nave_player.tempo_spawn_protecao_input = pygame.time.get_ticks() + 500
                    game_globals["jogador_esta_vivo_espectador"] = False; game_globals["jogador_pediu_para_espectar"] = False
                    game_globals["alvo_espectador_nome"] = None; camera.set_zoom(1.0); game_globals["spectator_overlay_hidden"] = False
                ...
                if nova_vida < nave_player.vida_atual: 
                    ...
                    if nave_player.nivel_escudo >= s.MAX_NIVEL_ESCUDO:
                         nave_player.mostrar_escudo_fx = True; nave_player.tempo_escudo_fx = pygame.time.get_ticks()
                ...
                if nave_player.nivel_max_vida > 0 and nave_player.nivel_max_vida < len(VIDA_POR_NIVEL): nave_player.max_vida = VIDA_POR_NIVEL[nave_player.nivel_max_vida]
                else: nave_player.max_vida = my_state.get('max_hp', nave_player.max_vida)
            
            if (my_state is None or nave_player.vida_atual <= 0) and (estado_jogo == "JOGANDO" or estado_jogo == "PVP_PLAYING"):
                if not game_globals["jogador_pediu_para_espectar"]:
                    ...
                    game_globals["alvo_espectador"] = None; game_globals["alvo_espectador_nome"] = None
                    espectador_dummy_alvo.posicao = nave_player.posicao.copy()
            if estado_jogo != "ESPECTADOR" and my_state: nave_player.rect.center = nave_player.posicao
            
        lista_alvos_naves = []
        if nave_player.vida_atual > 0 and not game_globals["jogador_esta_vivo_espectador"]: lista_alvos_naves.append(nave_player)
        ...
        if estado_jogo.startswith("PVP_"):
            ...
            else: lista_todos_alvos_para_aux = list(grupo_inimigos) + list(grupo_obstaculos) + list(grupo_bots)
        ...
        if not is_online:
                    if estado_jogo not in ["PAUSE"]:
                        ...
                        if estado_jogo.startswith("PVP_") or (estado_jogo == "ESPECTADOR" and is_pvp_map_offline):
                            ...
                            estado_jogo = novo_estado_jogo; game_globals["estado_jogo"] = novo_estado_jogo 
                            ...
                            if novo_estado_jogo == "ESPECTADOR" and estado_jogo != "ESPECTADOR":
                                estado_jogo = "ESPECTADOR"; game_globals["estado_jogo"] = "ESPECTADOR"
                                game_globals["jogador_esta_vivo_espectador"] = False
                                game_globals["alvo_espectador"] = None; game_globals["alvo_espectador_nome"] = None
                                game_globals["spectator_overlay_hidden"] = False
                                espectador_dummy_alvo.posicao = nave_player.posicao.copy()
        ...
        if estado_jogo not in ["PAUSE"]: grupo_efeitos_visuais.update() 
    ...
    if estado_jogo in ["JOGANDO", "LOJA", "TERMINAL", "PVP_LOBBY", "PVP_COUNTDOWN", "PVP_PLAYING", "PVP_PRE_MATCH"]:
        ...
        nave_player.update(grupo_projeteis_player, camera, socket_para_update, posicao_ouvinte_som, estado_jogo)

    online_data = {
        "players": online_players_copy, "npcs": online_npcs_copy, "projectiles": online_projectiles_copy
    }
    online_trackers = {
        "proj_last_frame": online_projectile_ids_last_frame, "npcs_last_frame": online_npcs_last_frame, "players_last_frame": online_players_last_frame
    }
    renderer.draw(estado_jogo, game_globals, game_groups, online_data, online_trackers, alvo_camera_final, posicao_ouvinte_som)
