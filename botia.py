...
class BotAI:
    ...
    def _processar_input_ia(self):
        ...
        # --- CORREÇÃO IMPORTANTE ---
        # Reseta o alvo de movimento do mouse. 
        # Se não fizermos isso, o bot ficará preso indo para o último ponto.
        self.bot.posicao_alvo_mouse = None
        
        # A mira (quer_virar_...) e o tiro (quer_atirar)
        # são controlados automaticamente por self.bot.alvo_selecionado
        # (Verifique ships.py, Nave.rotacionar() e Nave.lidar_com_tiros())
        ...
        if self.estado_ia == "EVITANDO_BORDA":
            if em_zona_segura:
                ...
                # Define o "clique do mouse" para o centro
        ...
        if (self.bot.vida_atual <= LIMITE_HP_FUGIR and is_alvo_ameaca) or self.estado_ia == "FUGINDO":
            ...
            try:
                ...
                # Define o "clique do mouse" para um ponto OPOSTO ao alvo
        ...
        if self.estado_ia == "COLETANDO": 
            # (No _encontrar_alvo, não travamos a mira em 'vida')
            # Precisamos encontrar o alvo de vida novamente aqui (mas é rápido)
            vida_perto = None
            dist_min = self.distancia_scan_geral
            
            # --- INÍCIO DA CORREÇÃO DO BUG ---
            # Acessa o grupo de vidas através de 'self.'
            ...
            if vida_perto:
                # Define o "clique do mouse" para a vida
        ...
        if self.estado_ia == "CAÇANDO":
            ...
            try:
                ...
                # Define o "clique do mouse" para o alvo (para avançar)
        ...
        if self.estado_ia == "ATACANDO":
            ...
            try:
                ...
                # LÓGICA DE MOVIMENTO (Kiting/Orbit)
                ponto_movimento = self.bot.posicao # Padrão: parar

                if distancia_alvo > self.distancia_orbita_max: 
                    # 1. Muito longe: Avança direto
                    ...
                    # 3. Na distância ideal: Orbita (Move para o lado)
                    if direcao_alvo_vec.length() > 0:
                        # Gira o vetor para o alvo em 75 graus (lado)
                        vetor_orbita = direcao_alvo_vec.rotate(75).normalize()
                        ponto_movimento = self.bot.posicao + vetor_orbita * 200
                
                # Define o "clique do mouse" para o ponto de movimento calculado
                self.bot.posicao_alvo_mouse = ponto_movimento
                
                # A mira (rotação) e o tiro são cuidados automaticamente
                # pela Nave, pois 'alvo_selecionado' está definido.
                return 
