...
from settings import (AZUL_NAVE, PONTA_NAVE, VERDE_AUXILIAR, LARANJA_BOT, 
                      MAX_TARGET_LOCK_DISTANCE, MAX_NIVEL_MOTOR, MAX_NIVEL_DANO, MAX_NIVEL_ESCUDO, 
...
def tocar_som_posicional(som, pos_fonte, pos_ouvinte, volume_base_config):
    if not som or not pos_ouvinte: return
    try: distancia = pos_fonte.distance_to(pos_ouvinte)
    except ValueError: distancia = 0 
    if distancia > MAX_DISTANCIA_SOM_AUDIVEL: return 
    fator_distancia = 1.0 - (distancia / MAX_DISTANCIA_SOM_AUDIVEL)
    volume_base = volume_base_config * fator_distancia; vetor_para_som = pos_fonte - pos_ouvinte; dist_x = vetor_para_som.x
    fator_pan = max(-1.0, min(1.0, dist_x / PANNING_RANGE_SOM))
    left_percent = (1.0 - fator_pan) / 2.0; right_percent = (1.0 + fator_pan) / 2.0
    left_vol = volume_base * left_percent; right_vol = volume_base * right_percent
    channel = pygame.mixer.find_channel() 
    if channel: channel.set_volume(left_vol, right_vol); channel.play(som)
...
class NaveAuxiliar(pygame.sprite.Sprite):
    ...
    def update(self, lista_alvos, grupo_projeteis_destino, estado_jogo_atual, nave_player_ref, client_socket=None, online_players={}, online_npcs={}, pos_ouvinte=None):
        ...
        if client_socket is None: 
            if not parar_ataque:
                ...
                if alvo_do_dono and alvo_do_dono.groups():
                    try:
                        ...
                        if dist < self.distancia_tiro: self.alvo_atual = alvo_do_dono
                    except ValueError: self.alvo_atual = None
                if self.alvo_atual:
                    ...
                    if agora - self.ultimo_tiro_tempo > self.cooldown_tiro:
                        ...
                        proj = ProjetilTeleguiadoJogador(self.posicao.x, self.posicao.y, radianos, self.owner.nivel_dano, owner_nave=self.owner, alvo_sprite=self.alvo_atual)
            ...
            alvo_do_dono_id = self.owner.alvo_selecionado; alvo_pos = None
            if alvo_do_dono_id:
                if alvo_do_dono_id in online_npcs:
                    alvo_state = online_npcs.get(alvo_do_dono_id)
                    if alvo_state and alvo_state.get('hp', 0) > 0: alvo_pos = pygame.math.Vector2(alvo_state['x'], alvo_state['y'])
                elif alvo_do_dono_id in online_players:
                    alvo_state = online_players.get(alvo_do_dono_id)
                    if alvo_state and alvo_state.get('hp', 0) > 0: alvo_pos = pygame.math.Vector2(alvo_state['x'], alvo_state['y'])
                if alvo_pos is None: self.owner.alvo_selecionado = None 
            if alvo_pos:
                try: direcao = (alvo_pos - self.posicao).normalize(); self.angulo = direcao.angle_to(pygame.math.Vector2(0, -1))
                except ValueError: self.angulo = self.owner.angulo
            else: self.angulo = self.owner.angulo
...
class Nave(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, cor=AZUL_NAVE, nome="Nave"):
        ...
        self.pontos_upgrade_disponiveis = 0; self.total_upgrades_feitos = 0; self._pontos_acumulados_para_upgrade = 0 
        self._limiares = PONTOS_LIMIARES_PARA_UPGRADE[:]; self._pontos_no_limiar = PONTOS_SCORE_PARA_MUDAR_LIMIAR[:]; self._limiar_pontos_atual = self._limiares[0]; self._indice_limiar = 0 
        ...
        self.cooldown_tiro = 250; self.ultimo_tiro_tempo = 0; self.pontos = 0; self.nivel_motor = 1; self.nivel_dano = 1; self.nivel_max_vida = 1; self.nivel_escudo = 0; self.nivel_aux = 0 
        self.max_vida = VIDA_POR_NIVEL[self.nivel_max_vida]; self.vida_atual = self.max_vida; self.velocidade_movimento_base = 4 + (self.nivel_motor * 0.5) 
    ...
    def update_regeneracao(self):
        if not self.esta_regenerando: return
        if self.quer_mover_frente or self.quer_mover_tras or self.posicao_alvo_mouse is not None: self.parar_regeneracao(); return
        if self.vida_atual >= self.max_vida: self.vida_atual = self.max_vida; self.parar_regeneracao(); return
        agora = pygame.time.get_ticks()
        if agora - self.ultimo_tick_regeneracao > REGEN_TICK_RATE:
            self.ultimo_tick_regeneracao = agora; self.vida_atual = min(self.max_vida, self.vida_atual + REGEN_POR_TICK); self.ultimo_hit_tempo = agora 
            print(f"[{self.nome}] Regenerou! Vida: {self.vida_atual:.1f}/{self.max_vida}") 
    def rotacionar(self):
        angulo_alvo = None; agora = pygame.time.get_ticks()
        if agora < self.tempo_fim_congelamento: return 
    ...
    def _gerar_particulas_rastro(self):
    ...
    def mover(self):
        ...
        nova_pos.x = max(meia_largura, min(nova_pos.x, s.MAP_WIDTH - meia_largura)); nova_pos.y = max(meia_altura, min(nova_pos.y, s.MAP_HEIGHT - meia_altura));
    ...
    def criar_projetil(self):
        ...
        if self.alvo_selecionado and self.alvo_selecionado.groups(): return ProjetilTeleguiadoJogador(pos_x, pos_y, radianos, self.nivel_dano, owner_nave=self, alvo_sprite=self.alvo_selecionado)
        else: return Projetil(pos_x, pos_y, radianos, self.nivel_dano, owner_nave=self)
    ...
    def foi_atingido(self, dano, estado_jogo_atual, proj_pos=None, atacante=None):
        if self.esta_regenerando: self.parar_regeneracao()
        if self.vida_atual <= 0 and estado_jogo_atual == "GAME_OVER": return False
        agora = pygame.time.get_ticks()
        if agora - self.ultimo_hit_tempo < 150: return False
        if atacante: self.ultimo_atacante = atacante 
    ...
    def aplicar_lentidao(self, duracao_ms): agora = pygame.time.get_ticks(); self.tempo_fim_lentidao = max(self.tempo_fim_lentidao, agora + duracao_ms); print(f"[{self.nome}] LENTIDÃO aplicada/estendida por {duracao_ms}ms!")
    def aplicar_congelamento(self, duracao_ms): agora = pygame.time.get_ticks(); self.tempo_fim_congelamento = max(self.tempo_fim_congelamento, agora + duracao_ms); print(f"[{self.nome}] CONGELADO por {duracao_ms}ms!")
    
    def ganhar_pontos(self, quantidade):
        if quantidade <= 0: return
        self.pontos += quantidade; self._pontos_acumulados_para_upgrade += quantidade
        while self._pontos_acumulados_para_upgrade >= self._limiar_pontos_atual:
            self.pontos_upgrade_disponiveis += 1; self._pontos_acumulados_para_upgrade -= self._limiar_pontos_atual 
            print(f"[{self.nome}] Ganhou 1 Ponto de Upgrade! (Total: {self.pontos_upgrade_disponiveis})")
            pontos_totais_aproximados = self.pontos 
            if self._indice_limiar < len(self._pontos_no_limiar) and pontos_totais_aproximados >= self._pontos_no_limiar[self._indice_limiar]:
                self._indice_limiar += 1
                if self._indice_limiar < len(self._limiares): self._limiar_pontos_atual = self._limiares[self._indice_limiar]; print(f"[{self.nome}] Próximo Ponto de Upgrade a cada {self._limiar_pontos_atual} pontos de score.")
    def comprar_auxiliar(self):
        num_ativos = len(self.grupo_auxiliares_ativos)
        if num_ativos < len(self.lista_todas_auxiliares):
            aux_para_adicionar = self.lista_todas_auxiliares[num_ativos]; offset_rotacionado = aux_para_adicionar.offset_pos.rotate(-self.angulo)
            posicao_correta_atual = self.posicao + offset_rotacionado
            aux_para_adicionar.posicao = posicao_correta_atual; aux_para_adicionar.rect.center = posicao_correta_atual; aux_para_adicionar.angulo = self.angulo 
            self.grupo_auxiliares_ativos.add(aux_para_adicionar); return True 
        else: return False
    def comprar_upgrade(self, tipo):
        if self.pontos_upgrade_disponiveis <= 0: print(f"[{self.nome}] Sem Pontos de Upgrade disponíveis!"); return False
        if self.total_upgrades_feitos >= MAX_TOTAL_UPGRADES: print(f"[{self.nome}] Limite máximo de {MAX_TOTAL_UPGRADES} upgrades atingido!"); return False
        comprou = False; custo_upgrade_atual = 1
        if tipo == "motor":
            if self.nivel_motor < MAX_NIVEL_MOTOR:
                if self.pontos_upgrade_disponiveis >= custo_upgrade_atual: self.pontos_upgrade_disponiveis -= custo_upgrade_atual; self.total_upgrades_feitos += 1; self.nivel_motor += 1; self.velocidade_movimento_base = 4 + (self.nivel_motor * 0.5); print(f"[{self.nome}] Motor comprado! Nível {self.nivel_motor}. ({self.pontos_upgrade_disponiveis} Pts Restantes)"); comprou = True
            ...
            if self.nivel_dano < MAX_NIVEL_DANO:
                if self.pontos_upgrade_disponiveis >= custo_upgrade_atual: self.pontos_upgrade_disponiveis -= custo_upgrade_atual; self.total_upgrades_feitos += 1; self.nivel_dano += 1; print(f"[{self.nome}] Dano comprado! Nível {self.nivel_dano}. ({self.pontos_upgrade_disponiveis} Pts Restantes)"); comprou = True
                else: print(f"[{self.nome}] Pontos insuficientes!")
            else: print(f"[{self.nome}] Nível máximo de dano atingido!")
        elif tipo == "auxiliar":
            num_ativos = len(self.grupo_auxiliares_ativos); max_aux = len(self.lista_todas_auxiliares) 
            if num_ativos < max_aux:
                custo_atual_aux = CUSTOS_AUXILIARES[num_ativos] 
                if self.pontos_upgrade_disponiveis >= custo_atual_aux:
                    if self.comprar_auxiliar(): self.pontos_upgrade_disponiveis -= custo_atual_aux; self.total_upgrades_feitos += 1; self.nivel_aux += 1; print(f"[{self.nome}] Auxiliar comprado por {custo_atual_aux} pts! ({self.pontos_upgrade_disponiveis} Pts Restantes)"); comprou = True
                else: print(f"[{self.nome}] Pontos insuficientes para Auxiliar! Custo: {custo_atual_aux} Pts. Você tem: {self.pontos_upgrade_disponiveis}")
            else: print(f"[{self.nome}] Nível máximo de auxiliares atingido!")
        elif tipo == "max_health":
            if self.nivel_max_vida >= len(VIDA_POR_NIVEL) - 1: print(f"[{self.nome}] Nível máximo de Vida Máx. atingido!"); return False 
            if self.pontos_upgrade_disponiveis >= custo_upgrade_atual: self.pontos_upgrade_disponiveis -= custo_upgrade_atual; self.total_upgrades_feitos += 1; self.nivel_max_vida += 1; self.max_vida = VIDA_POR_NIVEL[self.nivel_max_vida]; self.vida_atual += 1; self.ultimo_hit_tempo = pygame.time.get_ticks(); print(f"[{self.nome}] Vida Máx. aumentada! Nível {self.nivel_max_vida}. ({self.pontos_upgrade_disponiveis} Pts Restantes)"); comprou = True
            else: print(f"[{self.nome}] Pontos insuficientes!")
        elif tipo == "escudo":
            if self.nivel_escudo < MAX_NIVEL_ESCUDO:
                if self.pontos_upgrade_disponiveis >= custo_upgrade_atual: self.pontos_upgrade_disponiveis -= custo_upgrade_atual; self.total_upgrades_feitos += 1; self.nivel_escudo += 1; print(f"[{self.nome}] Escudo comprado! Nível {self.nivel_escudo}. ({self.pontos_upgrade_disponiveis} Pts Restantes)"); comprou = True
                else: print(f"[{self.nome}] Pontos insuficientes!")
    ...
    def desenhar_vida(self, surface, camera):
        ...
        if agora - self.ultimo_hit_tempo < self.tempo_barra_visivel:
            LARGURA_BARRA = 40; ALTURA_BARRA = 5; OFFSET_Y = 30; pos_x_mundo = self.posicao.x - LARGURA_BARRA / 2; pos_y_mundo = self.posicao.y - OFFSET_Y; percentual = max(0, self.vida_atual / self.max_vida); largura_vida_atual = LARGURA_BARRA * percentual
    ...
    def desenhar_nome(self, surface, camera):
        try:
            OFFSET_Y_VIDA_TOPO = 30; PADDING_ACIMA_VIDA = 3; pos_y_bottom_mundo = (self.posicao.y - OFFSET_Y_VIDA_TOPO) - PADDING_ACIMA_VIDA; pos_x_center_mundo = self.posicao.x
            texto_surf = FONT_NOME_JOGADOR.render(self.nome, True, BRANCO); texto_rect_mundo = texto_surf.get_rect(midbottom=(pos_x_center_mundo, pos_y_bottom_mundo)); surface.blit(texto_surf, camera.apply(texto_rect_mundo))
        except pygame.error as e: print(f"[ERRO] Falha ao renderizar nome '{self.nome}': {e}")
        except AttributeError: pass
            
class Player(Nave):
    def __init__(self, x, y, nome="Jogador"): super().__init__(x, y, cor=AZUL_NAVE, nome=nome); self.invencivel = False
    def foi_atingido(self, dano, estado_jogo_atual, proj_pos=None, atacante=None):
        if self.invencivel: return False 
        return super().foi_atingido(dano, estado_jogo_atual, proj_pos, atacante=atacante)
    def update(self, grupo_projeteis_jogador, camera, client_socket=None, pos_ouvinte=None, estado_jogo="JOGANDO"):
        self.processar_input_humano(camera, estado_jogo)
        if client_socket is None:
    ...
    def processar_input_humano(self, camera, estado_jogo="JOGANDO"):
        agora = pygame.time.get_ticks()
        if estado_jogo in ["PVP_PRE_MATCH", "LOJA", "TERMINAL"]: self.quer_virar_esquerda = False; self.quer_virar_direita = False; self.quer_mover_frente = False; self.quer_mover_tras = False; self.quer_atirar = False; self.posicao_alvo_mouse = None; return
        ...
        if agora > self.tempo_spawn_protecao_input:
            ...
            if mouse_buttons[0]:
                ...
                if not ui.RECT_BOTAO_UPGRADE_HUD.collidepoint(mouse_pos_tela) and not ui.RECT_BOTAO_REGEN_HUD.collidepoint(mouse_pos_tela):
                    mouse_pos_mundo = camera.get_mouse_world_pos(mouse_pos_tela); self.posicao_alvo_mouse = mouse_pos_mundo; self.quer_mover_frente = False; self.quer_mover_tras = False
            if self.quer_mover_frente or self.quer_mover_tras: self.posicao_alvo_mouse = None
        else: self.posicao_alvo_mouse = None; pygame.mouse.get_pressed()

class NaveBot(Nave):
    def __init__(self, x, y, dificuldade="Normal"):
        super().__init__(x, y, cor=LARANJA_BOT, nome=f"Bot {random.randint(1, 99)}"); self.cerebro = BotAI(self) 
        if dificuldade == "Dificil":
            self.nivel_motor = random.randint(1, MAX_NIVEL_MOTOR); self.velocidade_movimento_base = 4 + (self.nivel_motor * 0.5); self.nivel_dano = random.randint(1, MAX_NIVEL_DANO); self.nivel_escudo = random.randint(0, MAX_NIVEL_ESCUDO)
            max_spawn_vida_lvl = 3; self.nivel_max_vida = random.randint(1, max_spawn_vida_lvl); self.max_vida = VIDA_POR_NIVEL[self.nivel_max_vida]; self.vida_atual = self.max_vida
            max_aux = len(self.lista_todas_auxiliares); num_auxiliares = random.randint(0, max_aux)
            if num_auxiliares > 0:
                for _ in range(num_auxiliares): self.comprar_auxiliar(); self.nivel_aux += 1 
    def foi_atingido(self, dano, estado_jogo_atual, proj_pos=None, atacante=None):
        vida_antes = self.vida_atual; morreu = super().foi_atingido(dano, estado_jogo_atual, proj_pos, atacante=atacante)
        if morreu:
            is_pvp_map = (s.MAP_WIDTH < 5000) 
            if not is_pvp_map:
                print(f"[{self.nome}] BOT MORREU! Resetando...")
                novo_x = random.randint(50, s.MAP_WIDTH - 50); novo_y = random.randint(50, s.MAP_HEIGHT - 50); self.posicao = pygame.math.Vector2(novo_x, novo_y); self.rect.center = self.posicao
                self.grupo_auxiliares_ativos.empty(); self.lista_todas_auxiliares = [] 
                for pos in self.POSICOES_AUXILIARES: nova_aux = NaveAuxiliar(self, pos); self.lista_todas_auxiliares.append(nova_aux)
                self.pontos = 0; self.nivel_motor = 1; self.nivel_dano = 1; self.nivel_max_vida = 1; self.nivel_escudo = 0; self.nivel_aux = 0
                self.velocidade_movimento_base = 4 + (self.nivel_motor * 0.5); self.max_vida = VIDA_POR_NIVEL[self.nivel_max_vida]; self.vida_atual = self.max_vida
    ...
    def processar_upgrades_ia(self):
        if self.pontos_upgrade_disponiveis > 0 and self.total_upgrades_feitos < MAX_TOTAL_UPGRADES:
            ...
            elif self.nivel_escudo < MAX_NIVEL_ESCUDO: self.comprar_upgrade("escudo")
            elif self.nivel_dano < MAX_NIVEL_DANO: self.comprar_upgrade("dano")
            elif self.nivel_max_vida < len(VIDA_POR_NIVEL) - 1: self.comprar_upgrade("max_health")