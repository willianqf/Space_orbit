...
# ==================================================================================
# 2. FUNÇÕES AUXILIARES (LÓGICA PURA)
# ==================================================================================
...
def update_player_logic(player_state, lista_alvos_busca, agora_ms, map_width, map_height): 
    ...
    # Se estiver em PRE_MATCH, reseta alvo e retorna
    ...
    if (player_state['teclas']['space'] or player_state['alvo_lock']) and \
       (agora_ms - player_state['ultimo_tiro_tempo'] > player_state['cooldown_tiro']):
        ...
        proj = {
            ...
            'tipo': 'player_pvp' if player_state.get('is_pvp') else 'player_pve',
...
def update_npc_generic_logic(npc, players_dict, agora_ms):
    # Lógica simples para inimigos comuns
    ...
    if dist_min_sq < DISTANCIA_TIRO_PERSEGUIDOR_SQ: 
        if agora_ms - npc['ultimo_tiro_tempo'] > npc['cooldown_tiro']:
            ...
            return {
                'id': f"{npc['id']}_{agora_ms}", 'owner_nome': npc['id'],
                'x': npc['x'], 'y': npc['y'], 'pos_inicial_x': npc['x'], 'pos_inicial_y': npc['y'],
                'angulo_rad': radianos, 'velocidade': velocidade_proj, 'dano': 1, 
                'tipo': 'npc', 'tipo_proj': tipo_proj_npc, 'alvo_id': alvo_id_proj, 'timestamp_criacao': agora_ms,
...
def update_mothership_logic(npc, players_dict, agora_ms, room_ref):
    ...
    if npc.get('ia_estado') == 'VAGANDO':
        ...
        dist = math.sqrt(dx**2 + dy**2)
        if dist > 50:
            npc['x'] += (dx/dist) * 0.5; npc['y'] += (dy/dist) * 0.5
        ...
        if not target:
            ...
            if dist < 800: 
                npc['x'] += (dx/dist) * 1.0; npc['y'] += (dy/dist) * 1.0
...
def update_boss_congelante_logic(npc, players_dict, agora_ms, room_ref):
    ...
    if agora_ms - npc.get('ultimo_tiro_tempo', 0) > s.COOLDOWN_TIRO_CONGELANTE:
        ...
        return {
            'id': f"{npc['id']}_{agora_ms}", 'owner_nome': npc['id'],
            'x': npc['x'], 'y': npc['y'], 'pos_inicial_x': npc['x'], 'pos_inicial_y': npc['y'],
            'dano': 1, 'tipo': 'npc', 'tipo_proj': 'congelante', 'velocidade': VELOCIDADE_PROJ_CONGELANTE,
            'vel_x': vel_x, 'vel_y': vel_y, 'alvo_id': target['nome'], 'timestamp_criacao': agora_ms
        }
    return None
    ...
    if target and agora_ms - npc.get('ultimo_tiro_tempo', 0) > npc['cooldown_tiro']:
         ...
         if d_target < 400**2:
             ...
             return {
                'id': f"{npc['id']}_{agora_ms}", 'owner_nome': npc['id'],
                'x': npc['x'], 'y': npc['y'], 'pos_inicial_x': npc['x'], 'pos_inicial_y': npc['y'],
                'dano': 1, 'tipo': 'npc', 'tipo_proj': 'normal', 'velocidade': VELOCIDADE_PROJETIL_NPC,
                'vel_x': vel_x, 'vel_y': vel_y, 'angulo_rad': rad_tiro
            }
    return None
...
def server_spawnar_inimigo_aleatorio(x, y, npc_id):
    ...
    if chance < 0.05: 
        tipo = "bomba"; hp, max_hp = 1, 1; tamanho = 25; cooldown_tiro = 999999; pontos = 3
    elif chance < 0.10: 
        tipo = "tiro_rapido"; hp, max_hp = 10, 10; tamanho = 30; cooldown_tiro = 1500; pontos = 20
    elif chance < 0.15: 
        tipo = "atordoador"; hp, max_hp = 5, 5; tamanho = 30; cooldown_tiro = 5000; pontos = 25
    elif chance < 0.35: 
        tipo = "atirador_rapido"; hp, max_hp = 1, 1; tamanho = 30; cooldown_tiro = 500; pontos = 10
    elif chance < 0.55: 
        tipo = "rapido"; hp, max_hp = 5, 5; tamanho = 30; cooldown_tiro = 800; pontos = 9
...
def server_spawnar_boss_congelante(x, y, npc_id):
    return {
        ...
        'ia_ultimo_spawn_minion': 0, 'ia_ultimo_hit_tempo': 0,
        'ia_wander_target': None 
    }

def server_spawnar_minion_mothership(owner, target_id, index, max_minions, npc_id_num):
    return {
        'id': f"minion_ms_{npc_id_num}", 'tipo': 'minion_mothership', 'owner_id': owner['id'],
        'x': owner['x'], 'y': owner['y'], 'angulo': 0.0,
        'hp': 2, 'max_hp': 2, 'tamanho': 15, 'pontos_por_morte': 1,
        'cooldown_tiro': 1000, 'ultimo_tiro_tempo': 0,
        'ia_alvo_id': target_id, 'ia_raio_orbita': owner['tamanho'] * 0.8 + random.randint(30, 60),
...
def server_spawnar_minion_congelante(owner, target_id, index, max_minions, npc_id_num):
    return {
        'id': f"minion_bc_{npc_id_num}", 'tipo': 'minion_congelante', 'owner_id': owner['id'],
        'x': owner['x'], 'y': owner['y'], 'angulo': 0.0,
        ...
        'ia_alvo_id': target_id, 'ia_raio_orbita': owner['tamanho'] * 0.7 + random.randint(30, 50),
        'ia_angulo_orbita': (index / max_minions) * 360, 'ia_vel_orbita': random.uniform(0.5, 1.0) 
    }
    ...
    for state in player_dict.values():
        if state.get('handshake_completo', True): 
            ...
            estado_str = (
                ...
                f":{is_lento}:{is_congelado}:{is_pre_match}"
            )
...
class GameRoom:
    ...
    def broadcast(self, message_bytes):
        dead_conns = []
        with self.lock:
            recipients = [k for k in self.players.keys() if isinstance(k, socket.socket)]
    ...
    def remove_player(self, conn):
        with self.lock:
            if conn in self.players:
                print(f"[SALA {self.room_id}] {self.players[conn]['nome']} saiu.")
                del self.players[conn]

    def is_full(self):
        with self.lock:
            return len([k for k in self.players if isinstance(k, socket.socket)]) >= self.max_players
...
class PveRoom(GameRoom):
    ...
    def update(self):
        ...
        with self.lock:
            bots_remover = self.bot_manager.manage_bot_population(s.MAX_BOTS)
    ...
    def _update_logic(self):
                ...
                if target:
                    for i in range(p['nivel_aux']):
                        if self.agora_ms > p['aux_cooldowns'][i]:
                            ...
                            if (ax-target['x'])**2 + (ay-target['y'])**2 < AUX_DISTANCIA_TIRO_SQ:
                                p['aux_cooldowns'][i] = self.agora_ms + AUX_COOLDOWN_TIRO
                                rad = math.radians(p['angulo'])
                                novos_projeteis.append({
                                    'id': f"{p['nome']}_aux{i}_{self.agora_ms}", 'owner_nome': p['nome'],
                                    'x': ax, 'y': ay, 'pos_inicial_x': ax, 'pos_inicial_y': ay,
                                    'dano': s.DANO_POR_NIVEL[p['nivel_dano']], 'tipo': 'player_pve', 'tipo_proj': 'teleguiado',
                                    'velocidade': 14, 'alvo_id': t_id, 'timestamp_criacao': self.agora_ms,
                                    'vel_x': -math.sin(rad)*14, 'vel_y': -math.cos(rad)*14
                                })
        ...
        for npc in self.npcs:
            ...
            if npc['tipo'] == 'mothership':
                update_mothership_logic(npc, self.players, self.agora_ms, self)
            elif npc['tipo'] == 'boss_congelante':
                new_proj = update_boss_congelante_logic(npc, self.players, self.agora_ms, self)
            elif 'minion' in npc['tipo']:
                new_proj = update_minion_logic(npc, self.players, self.agora_ms, self)
            else:
                new_proj = update_npc_generic_logic(npc, self.players, self.agora_ms)
        ...
        for proj in self.projectiles:
            ...
            if (proj['x']-proj['pos_inicial_x'])**2 + (proj['y']-proj['pos_inicial_y'])**2 > MAX_DISTANCIA_TIRO_SQ:
                toremove_proj.append(proj); continue
            ...
            if proj['tipo'] == 'player_pve':
                ...
                for npc in self.npcs:
                    ...
                    if (npc['x']-proj['x'])**2 + (npc['y']-proj['y'])**2 < (npc['tamanho']/2 + 10)**2:
                        npc['hp'] -= proj['dano']
                        npc['ia_ultimo_hit_tempo'] = self.agora_ms # IA reage ao dano
                        hit = True
                        if npc['hp'] <= 0:
                ...
                # Colisão com Outros Players/Bots
                if not hit:
                    for target in living_players:
                        ...
                        if (target['x']-proj['x'])**2 + (target['y']-proj['y'])**2 < COLISAO_JOGADOR_PROJ_DIST_SQ:
                            dano_base = proj['dano']
                            fator_escudo = min(target['nivel_escudo'] * REDUCAO_DANO_POR_NIVEL, 75) / 100.0
                            target['hp'] -= dano_base * (1.0 - fator_escudo)
                ...
                for p in living_players:
                    if (p['x']-proj['x'])**2 + (p['y']-proj['y'])**2 < COLISAO_JOGADOR_PROJ_DIST_SQ:
                        dano = 1 * (1 - min(p['nivel_escudo'] * REDUCAO_DANO_POR_NIVEL / 100.0, 0.75))
                        p['hp'] -= dano
        ...
        if living_players:
             ...
             if count_normais < s.MAX_INIMIGOS:
                 refs = [(p['x'], p['y']) for p in living_players]
                 sx, sy = server_calcular_posicao_spawn(refs, s.MAP_WIDTH, s.MAP_HEIGHT)
                 self.npcs.append(server_spawnar_inimigo_aleatorio(sx, sy, f"npc_{self.next_npc_id}"))
                 self.next_npc_id += 1
             
             # Mothership
             count_ms = sum(1 for n in self.npcs if n['hp'] > 0 and n['tipo'] == 'mothership')
             if count_ms < s.MAX_MOTHERSHIPS:
                 refs = [(p['x'], p['y']) for p in living_players]
                 sx, sy = server_calcular_posicao_spawn(refs, s.MAP_WIDTH, s.MAP_HEIGHT)
                 self.npcs.append(server_spawnar_mothership(sx, sy, f"ms_{self.next_npc_id}"))
                 self.next_npc_id += 1

             # Boss Congelante
             count_bc = sum(1 for n in self.npcs if n['hp'] > 0 and n['tipo'] == 'boss_congelante')
             if count_bc < s.MAX_BOSS_CONGELANTE:
                 refs = [(p['x'], p['y']) for p in living_players]
                 sx, sy = server_calcular_posicao_spawn(refs, s.MAP_WIDTH, s.MAP_HEIGHT)
                 self.npcs.append(server_spawnar_boss_congelante(sx, sy, f"bc_{self.next_npc_id}"))
                 self.next_npc_id += 1
...
class PvpRoom(GameRoom):
    def __init__(self, room_id):
        ...
        self.timer_fim = 0
        self.timer_partida = 0
        self.winner = ""
        self.restart_ts = 0

    def reset_match(self):
        print(f"[SALA {self.room_id}] Resetando PVP.")
        self.lobby_state = "WAITING"
        self.winner = ""
        self.projectiles.clear()
        for p in self.players.values():
            p['x'] = pvp_s.SPAWN_LOBBY.x; p['y'] = pvp_s.SPAWN_LOBBY.y
            p['hp'] = p['max_hp']; p['is_pre_match'] = False; p['esta_regenerando'] = False; p['alvo_lock'] = None
        ...
        with self.lock:
            ...
            if self.lobby_state == "WAITING":
                ...
                if len(self.players) >= 4: 
                    self.lobby_state = "COUNTDOWN"; self.timer_fim = self.agora_ms + LOBBY_COUNTDOWN_MS
            
            elif self.lobby_state == "COUNTDOWN":
                self._update_physics_pvp()
                if len(self.players) < 4: self.lobby_state = "WAITING"
                elif self.agora_ms > self.timer_fim:
                    self.lobby_state = "PRE_MATCH"
                    self.timer_fim = self.agora_ms + PRE_MATCH_FREEZE_MS
                    self.timer_partida = self.agora_ms + PRE_MATCH_FREEZE_MS + PARTIDA_COUNTDOWN_MS
                    lista = list(self.players.values())
                    for i, p in enumerate(lista):
                        if i < len(pvp_s.SPAWN_POSICOES):
                            p['x'] = pvp_s.SPAWN_POSICOES[i].x; p['y'] = pvp_s.SPAWN_POSICOES[i].y
                ...
                if self.agora_ms > self.timer_fim:
                ...
                if len(vivos) <= 1 or self.agora_ms > self.timer_partida:
                    self.lobby_state = "GAME_OVER"
                    self.winner = vivos[0]['nome'] if vivos else "Empate"
                    self.restart_ts = self.agora_ms + RESTART_DELAY_MS
            elif self.lobby_state == "GAME_OVER":
                if self.agora_ms > self.restart_ts:
                    if len(self.players) > 0: self.reset_match()
    ...
    def _update_physics_pvp(self):
        ...
        for p in living:
            ...
            # [ADICIONADO] Lógica de Auxiliares no PVP
            if p.get('nivel_aux', 0) > 0 and p.get('alvo_lock'):
                t_id = p['alvo_lock']
                target = next((x for x in living if x.get('nome') == t_id), None)
                if target:
                    for i in range(p['nivel_aux']):
                        if self.agora_ms > p['aux_cooldowns'][i]:
                            off_x, off_y = _rotate_vector(AUX_POSICOES[i][0], AUX_POSICOES[i][1], -p['angulo'])
                            ...
                            if (ax-target['x'])**2 + (ay-target['y'])**2 < AUX_DISTANCIA_TIRO_SQ:
                                ...
                                self.projectiles.append({
                                    'id': f"{p['nome']}_aux{i}_{self.agora_ms}", 'owner_nome': p['nome'],
                                    'x': ax, 'y': ay, 'pos_inicial_x': ax, 'pos_inicial_y': ay,
                                    'dano': s.DANO_POR_NIVEL[p['nivel_dano']], 'tipo': 'player_pvp', 'tipo_proj': 'teleguiado',
                                    'velocidade': 14, 'alvo_id': t_id, 'timestamp_criacao': self.agora_ms,
                                    'vel_x': -math.sin(rad)*14, 'vel_y': -math.cos(rad)*14
                                })
...
def handle_client(conn, addr):
    ...
    try:
        ...
        parts = data.split('|')
        name_req = parts[0]; mode = parts[1] if len(parts) > 1 else "PVE"
        
        room = find_room(mode)
        if not room:
            conn.sendall(b"REJEITADO|Sem salas\n"); conn.close(); return
        
        name = name_req
        with room.lock:
            exist = [p['nome'] for p in room.players.values()]
            i=1
        ...
        p_state = {
            'conn': conn, 'nome': name, 'is_bot': False, 'is_pvp': (mode=="PVP"),
            'handshake_completo': True, 'x': sx, 'y': sy, 'angulo': 0,
            'hp': float(s.VIDA_POR_NIVEL[1]), 'max_hp': float(s.VIDA_POR_NIVEL[1]),
            ...
            'pontos': 0, 'pontos_upgrade_disponiveis': (10 if mode=="PVP" else 0),
            'total_upgrades_feitos': 0, '_pontos_acumulados_para_upgrade': 0, 
            '_limiar_pontos_atual': PONTOS_LIMIARES_PARA_UPGRADE[0], '_indice_limiar': 0,
            'nivel_motor': 1, 'nivel_dano': 1, 'nivel_max_vida': 1, 'nivel_escudo': 0, 'nivel_aux': 0,
            'aux_cooldowns': [0]*4
        }
        ...
        print(f"[JOIN] {name} -> {room.room_id}")
        ...
        while True:
            ...
            with room.lock:
                ...
                if me['hp'] <= 0 and not me['is_pvp'] and "RESPAWN_ME" in lines:
                        refs = [(p['x'], p['y']) for p in room.players.values()]
                        rx, ry = server_calcular_posicao_spawn(refs, s.MAP_WIDTH, s.MAP_HEIGHT)
                        me['x'] = rx; me['y'] = ry; me['hp'] = me['max_hp']
                        me['alvo_lock'] = None; me['pontos'] = 0
                        continue
                ...
                for l in lines:
                    if l == "W_DOWN": me['teclas']['w'] = True; me['alvo_mouse'] = None
                    ...
                    elif l.startswith("CLICK_MOVE|"):
                        _, x, y = l.split('|'); me['alvo_mouse'] = (int(x), int(y))
                        me['teclas']['w'] = False; me['teclas']['s'] = False
                    elif l.startswith("CLICK_TARGET|"):
                        _, cx, cy = l.split('|'); cx=int(cx); cy=int(cy)
                        ...
                        for t in targets:
                            ...
                            if dist < TARGET_CLICK_SIZE_SQ and dist < best_d:
                                best_d = dist; best_id = t.get('id', t.get('nome'))
                    ...
                    elif l.startswith("BUY_UPGRADE|"):
                        server_comprar_upgrade(me, l.split('|')[1])
                    elif l == "ENTER_SPECTATOR":
                        me['hp'] = 0
...
def server_loop():
    while True:
        ...
        for r in all_rooms:
            r.update()
            r.broadcast(r.get_state_bytes())
        dt = time.time() - st
        wait = (1.0/TICK_RATE) - dt
...
def main():
    ...
    sock.listen(32)
    print(f"Server {HOST}:{PORT} | PVE: {QTD_SALAS_PVE} | PVP: {QTD_SALAS_PVP}")
    ...
    threading.Thread(target=connection_listener, args=(sock,), daemon=True).start()
    ...
    try:
        while True:
            ...
            if cmd == "status":
                for r in all_rooms:
                    print(f"[{r.room_id}] {len(r.players)}/{r.max_players} players")
